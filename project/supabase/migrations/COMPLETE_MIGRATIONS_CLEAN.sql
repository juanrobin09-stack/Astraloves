-- ═══════════════════════════════════════════════════════════════════════
-- ASTRA - MIGRATIONS SQL COMPLÈTES (AVEC NETTOYAGE)
-- Date: 2026-01-10
-- Description: Système complet d'abonnements et résultats
-- ═══════════════════════════════════════════════════════════════════════

-- ┌─────────────────────────────────────────────────────────────────────┐
-- │ 0. NETTOYAGE DES TABLES EXISTANTES                                  │
-- └─────────────────────────────────────────────────────────────────────┘

-- Supprimer les tables dans le bon ordre (foreign keys)
...
DROP FUNCTION IF EXISTS reset_daily_usage_if_needed() CASCADE;
DROP FUNCTION IF EXISTS initialize_daily_usage() CASCADE;
DROP FUNCTION IF EXISTS reset_all_daily_usage() CASCADE;
...
CREATE INDEX idx_daily_usage_last_reset ON daily_usage(last_reset);
...
-- Index
CREATE INDEX idx_astral_themes_user_id ON astral_themes(user_id);
...
-- Index
CREATE INDEX idx_insights_history_user_id ON insights_history(user_id);
CREATE INDEX idx_insights_history_type ON insights_history(type);
CREATE INDEX idx_insights_history_created_at ON insights_history(created_at);
...
CREATE FUNCTION reset_daily_usage_if_needed()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE FUNCTION initialize_daily_usage()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE FUNCTION reset_all_daily_usage()
RETURNS void AS $$
BEGIN
  UPDATE daily_usage
  SET 
